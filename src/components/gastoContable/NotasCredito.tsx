...
const NotasCredito = () => {
  ...
  const [notasCredito, setNotasCredito] = useState<ListarGastoContableProps[]>(
    []
  );
  ...
  const [gastoContableRegistrar, setGastoContableRegistrar] =
    useState<GastoContableRegistrarProps>({
      ...
      usuario: "",
      proveedorId: "",
      gastoId: "5", //? valor por defecto (Notas de Crédito)
    });
  ...
  const getNotasCredito = useCallback(async () => {
    if (!token || !hasPermission) return;
    try {
      const data = await listarGastosContables(
        token,
        "5", // GastoId para Notas de Crédito
        selectedMonth,
        selectedYear
      );
      ...
      toast.error("Error al obtener las notas de crédito", {
        theme: "colored",
      });
    }
    ...
    <>
      ...
      <div className="w-full p-4 border rounded-lg bg-white dark:bg-neutral-900 shadow-sm">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          ...
          <div className="flex flex-wrap justify-end gap-3 w-full md:w-auto">
            <Link href="/dashboard/main" className="w-full md:w-auto">
            ...
            <Button
              ...
              onClick={() => handleExportExcel(notasCredito)}
            >
      ...
      <div className="overflow-x-auto bg-white shadow-lg rounded-lg p-4 mt-6 dark:bg-neutral-900 border">
        <Table className="w-full">
          <TableCaption>Listado de Notas de Crédito</TableCaption>
          <TableHeader>
            <TableRow>
